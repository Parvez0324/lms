...
<div class="h-screen flex overflow-hidden bg-slate-50" 
    ...
    <aside :class="{ 
        ...
        <div class="flex-1 px-2.5 py-2.5 overflow-y-auto space-y-0.5">
            ...
            <div x-show="!sidebarCollapsed" class="px-2.5 pt-1.5 pb-0.5 text-[9px] font-bold uppercase tracking-wider text-slate-400">Overview</div>
            <div x-show="sidebarCollapsed" class="my-1 border-t border-slate-100"></div>
            
            <a href="{{ route('admin.dashboard') }}" 
               :title="sidebarCollapsed ? 'Dashboard' : ''"
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
            ...
            <a href="{{ route('admin.reports.index') }}" 
               ...
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.reports.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
            ...
            <div x-show="!sidebarCollapsed" class="pt-2.5 px-2.5 pb-0.5 text-[9px] font-bold uppercase tracking-wider text-slate-400">Curriculum</div>
            <div x-show="sidebarCollapsed" class="my-1 border-t border-slate-100"></div>

            <a href="{{ route('admin.categories.index') }}" 
               :title="sidebarCollapsed ? 'Categories' : ''"
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.categories.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
            ...
            <a href="{{ route('admin.courses.index') }}" 
               ...
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.courses.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
                <i data-lucide="book-open-check" class="w-4 h-4 text-slate-500 flex-shrink-0"></i>
                <span x-show="!sidebarCollapsed" class="truncate">All Courses</span>
            </a>

            <a href="{{ route('admin.lessons.index') }}" 
               :title="sidebarCollapsed ? 'Lessons & Media' : ''"
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.lessons.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
                <i data-lucide="video" class="w-4 h-4 text-slate-500 flex-shrink-0"></i>
                <span x-show="!sidebarCollapsed" class="truncate">Lessons & Media</span>
            </a>

            <a href="{{ route('admin.quizzes.index') }}" 
               :title="sidebarCollapsed ? 'Quizzes & Bank' : ''"
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.quizzes.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
                <i data-lucide="help-circle" class="w-4 h-4 text-slate-500 flex-shrink-0"></i>
                <span x-show="!sidebarCollapsed" class="truncate">Quizzes & Bank</span>
            </a>

            <a href="{{ route('admin.assignments.index') }}" 
               :title="sidebarCollapsed ? 'Assignments' : ''"
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.assignments.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
            ...
            <div x-show="!sidebarCollapsed" class="pt-2.5 px-2.5 pb-0.5 text-[9px] font-bold uppercase tracking-wider text-slate-400">People & Sales</div>
            <div x-show="sidebarCollapsed" class="my-1 border-t border-slate-100"></div>

            <a href="{{ route('admin.students.index') }}" 
               :title="sidebarCollapsed ? 'Students' : ''"
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.students.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
                <i data-lucide="users" class="w-4 h-4 text-slate-500 flex-shrink-0"></i>
                <span x-show="!sidebarCollapsed" class="truncate">Students</span>
            </a>

            <a href="{{ route('admin.instructors.index') }}" 
               :title="sidebarCollapsed ? 'Instructors' : ''"
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.instructors.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
                <i data-lucide="sparkles" class="w-4 h-4 text-slate-500 flex-shrink-0"></i>
                <span x-show="!sidebarCollapsed" class="truncate">Instructors</span>
            </a>

            <a href="{{ route('admin.enrollments.index') }}" 
               :title="sidebarCollapsed ? 'Enrollments' : ''"
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.enrollments.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
                <i data-lucide="user-check" class="w-4 h-4 text-slate-500 flex-shrink-0"></i>
                <span x-show="!sidebarCollapsed" class="truncate">Enrollments</span>
            </a>

            <a href="{{ route('admin.payments.index') }}" 
               :title="sidebarCollapsed ? 'Payments & Sales' : ''"
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.payments.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
            ...
            <a href="{{ route('admin.certificates.index') }}" 
               ...
               class="flex items-center rounded-lg text-xs font-semibold transition-all {{ request()->routeIs('admin.certificates.*') ? 'bg-rose-50 text-rose-700 shadow-2xs shadow-rose-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50' }}"
               :class="sidebarCollapsed ? 'justify-center px-0 py-1.5' : 'space-x-2.5 px-2.5 py-1.5'">
