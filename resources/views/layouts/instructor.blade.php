        ...
        <header class="h-14 bg-white border-b border-slate-200/90 px-4 sm:px-6 flex items-center justify-between flex-shrink-0 z-10">
            <div class="flex items-center space-x-3">
                ...
                <div class="hidden sm:flex items-center space-x-2 text-xs font-semibold text-slate-500">
                    <span class="text-slate-400">Instructor Studio</span>
                    <i data-lucide="chevron-right" class="w-3.5 h-3.5 text-slate-300"></i>
                    <span class="text-slate-800 font-bold capitalize">{{ str_replace(['instructor.', '.'], ['', ' > '], request()->route()->getName() ?: 'Dashboard') }}</span>
                </div>
            </div>

            <div class="flex items-center space-x-3">
                <a href="{{ route('home') }}" class="hidden sm:inline-flex items-center space-x-1 px-3 py-1.5 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100">
                ...
                <div class="relative" x-data="{ open: false }" @click.away="open = false">
                    <button @click="open = !open" class="flex items-center space-x-2 p-1.5 rounded-xl hover:bg-slate-100 focus:outline-none">
                        <img class="w-8 h-8 rounded-xl object-cover ring-2 ring-brand-500/20" src="{{ Auth::user()->avatar_url }}" alt="">
                    ...
                    <div x-show="open" class="absolute right-0 mt-2 w-56 rounded-2xl bg-white shadow-xl ring-1 ring-black/5 py-1.5 z-50 divide-y divide-slate-100 text-xs" style="display: none;">
                        ...
                        <div class="py-1">
                            ...
                            <a href="{{ route('student.dashboard') }}" class="flex items-center px-4 py-2 text-slate-700 hover:bg-slate-50 font-medium">
                                <i data-lucide="book-open" class="w-4 h-4 mr-2 text-emerald-500"></i> Student Dashboard
                            </a>
                        </div>
                        <div class="py-1">
