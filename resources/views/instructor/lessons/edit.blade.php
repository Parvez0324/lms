...
<div class="max-w-xl mx-auto space-y-4" x-data="{ contentType: '{{ $lesson->content_type }}' }">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-black text-slate-900 tracking-tight">Edit Lesson</h1>
            <p class="text-xs text-slate-500">Section: {{ $lesson->section->title }}</p>
        </div>
        <a href="{{ route('instructor.courses.show', $lesson->section->course_id) }}" class="text-xs font-bold text-slate-500 hover:text-slate-800 flex items-center space-x-1">
    ...
    <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
        <form action="{{ route('instructor.lessons.update', $lesson->id) }}" method="POST" enctype="multipart/form-data" class="space-y-3.5">
            @csrf
            @method('PUT')
            ...
            <div x-show="contentType === 'video'" class="space-y-2">
                ...
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-700 mb-0.5">Upload New Video File (.mp4)</label>
            ...
            <div x-show="contentType === 'pdf'" style="display: none;">
                <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-700 mb-0.5">Upload New PDF File</label>
            ...
            <div x-show="contentType === 'article'" style="display: none;">
                <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-700 mb-0.5">Article Content</label>
                <textarea name="article_content" rows="4" 
                          class="w-full px-3 py-1.5 text-xs rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-brand-500">{{ old('article_content', $lesson->article_content) }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-700 mb-0.5">Duration (Mins)</label>
                    <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $lesson->duration_minutes) }}" min="1" required 
                           class="w-full px-3 py-1.5 text-xs rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-brand-500">
                </div>

                <div class="flex items-center pt-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_preview" value="1" {{ $lesson->is_preview ? 'checked' : '' }} class="h-3.5 w-3.5 text-brand-600 rounded border-slate-300">
            ...
            <div class="pt-2 border-t border-slate-100 flex justify-end space-x-2">
                <a href="{{ route('instructor.courses.show', $lesson->section->course_id) }}" class="px-4 py-2 text-xs font-bold text-slate-500 rounded-xl hover:bg-slate-100">Cancel</a>
                <button type="submit" class="px-5 py-2 bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs rounded-xl shadow-md">Save Changes</button>
            </div>
        </form>
    </div>
</div>
